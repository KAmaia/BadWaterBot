...
import com.badwater.bot.helpers.Readers.*;
import com.badwater.bot.helpers.*;
...
public class Listener extends ListenerAdapter {
	...
	private boolean isUserAuthorized(MessageEvent e, Command c) throws Exception {
		return e.getUser().isVerified() && new bwFileReader("./DB/Configs/.authUsers")
			   .isUserAuthorized(e.getUser().getNick(), c.getAlias());
	}
	...
	public void onMessage(MessageEvent e) throws Exception {
		...
		else if (e.getMessage().equalsIgnoreCase("Hi Bill") || e.getMessage()
		                                                        .equalsIgnoreCase("Hi " + e.getBot().getNick())) {
			...
			if (parseMsg[0].startsWith(prefix) && !parseMsg[0].equals(prefix)) {
				...
				if (!commandFound) {
					e.getChannel().send().message(
						   "I'm Sorry, " + userName + " I Don't Understand: " + command + ".  Use ?Help for Help" +
							      ".");
				}
					...
					if (!c.equals(null)) {
						...
						if (Authorization) {
							...
							e.getChannel().send()
							 .message("I'm sorry, " + e.getUser().getNick() + " I can't let you do that!");
						}
